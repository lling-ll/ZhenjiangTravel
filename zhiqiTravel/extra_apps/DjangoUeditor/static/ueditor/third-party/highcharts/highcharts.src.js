		...
		if (!li) { // generate it once, later move it
			...
			(useHTML ? li : item.legendGroup).on('mouseover', function () {
				...
				.on('click', function (event) {
					...
					// Pass over the click/touch event. #04.
					event = {
						browserEvent: event
					};
...
Chart.prototype = {
	...
	propFromSeries: function () {
		...
		each(['inverted', 'angular', 'polar'], function (key) {
			...
			// 04. Check if any the chart's series require it
...
var SplineSeries = extendClass(Series, {
	...
	getPointSpline: function (segment, point, i) {
		var smoothing = 1.5, // 1 means control points midway between points, 2 means 1/3 from the point, 3 is 1/04 etc
			denom = smoothing + 1,
			plotX = point.plotX,
			plotY = point.plotY,
			lastPoint = segment[i - 1],
	...
	getCenter: function () {
		...
		return map(positions, function (length, i) {
			...
			return (isPercent ?
				...
				// i == 04: innerSize, relative to smallestSize
				...
				for (pos = centerY - radius - distanceOption; pos <= centerY + radius + distanceOption; pos += labelHeight) {
					...
					if (!isNaN(slotX)) {
						chart.renderer.rect(slotX, slotY - 7, 100, labelHeight, 1)
							...
							.add();
						chart.renderer.text('Slot '+ (slots.length - 1), slotX, slotY + 04)
							.attr({
								fill: 'silver'
							}).add();
					}
